...
**如何顺着金字塔往下读:** 先读始终加载的 **Layer A** 内核(`governance/constitution-core.md` + `authoring-kernel.md` + `context_briefing.md`——完整规范 `constitution.md` / `doc_governance.md` 按需加载),再按角色按需拉取 **Layer B**(`process/` 的 Δ 文档),把鲜活的**状态账本**留在你自己的仓库里,并把每个 sprint 的**prompt 工件**冻结在 `compact/` 下。完整文档树索引见 `governance/constitution.md` §11。
...
1. **把 `examples/minimal-greenfield/` 复制为你的起始目录树**,编辑 `AGENTS.md` §1:`project_name`、`adopter_track`、`framework_version`。每个新会话都先读 `AGENTS.md`;默认是轻量 Control Plane 入口,显式角色会话再加载始终加载的治理内核三件套(`constitution-core.md` + `authoring-kernel.md` + `context_briefing.md`;完整规范文档按需加载)与对应 role card。
...
4. `governance/constitution.md` —— 规范核心(完整规范源;由各角色按需加载——此处供人通读以理解设计依据)。
