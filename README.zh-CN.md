...
- **治理内核** —— 冷启动恒加载的是内核三件套(`constitution-core.md` + `authoring-kernel.md` + `context_briefing.md`),硬约束由其投影自 `constitution.md`(唯一规范源头;完整权威文档按需加载);charter 校验器会**拒绝任何 checkpoint 绕过,并直接拒绝启动**。
...
| **kernel / constitution-core** | 冷启动时持有的、宪法硬约束的恒加载精简投影(完整宪法按需加载) |
