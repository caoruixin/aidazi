...
- **Status:** DESIGN SPEC rev3 — **Codex gpt-5.5 xhigh APPROVE (R3, `archive/2026-06-30-track2-codex-review-r3.md`)**. Path: R1 `REVISE` (B1–B5)→rev2→R2 confirmed B1–B5 RESOLVED + inventory COMPLETE, REVISE on TD6 dual-hash→rev3 TD6 redesign→R3 APPROVE (TD6 SOUND, no new blocking; 2 impl nits folded §2.1-R3). Reviews: `…-track2-codex-review-r1.md`, `-r2.md`, `-r3.md`. **Implementation blocker for OW-M3** (`archive/2026-06-30-ow-m3-mandatory-e2e-spec.md` §5); home for the pre-existing Track-2 authz gap ([[track2-gap-followup-signing-followup]]).
...
- **TD6 implementation precision (R3 nits + Part-A guard spec):**
  - **Exact-diff guard (R3-sharpened):** the re-stamp validates the live envelope against the stored signed `scope_envelope`/`H` and permits ONLY a single insertion into the CURRENT milestone's `subsprint_sequence` at cursor+1, with the prefix and suffix UNCHANGED and NO other bound-field delta anywhere. Any prefix/suffix reorder, a multi-item or non-cursor+1 edit, a prompt-id swap, or any other authority-field change ⟹ refuse ⟹ stale ⟹ blocked. (A Customer change paired with an insertion is rejected because the "no other bound-field delta" clause fails.)
  - **Atomic re-stamp (R3 nit #1):** the re-stamp updates `signoff.scope_envelope` AND `signed_scope_hash` AND the `engine_authored_delta` provenance together, atomically — else `signoff_snapshot_authentic()`/scope reporting fail closed after a re-stamp.
  - **Ordering (R3 nit #2):** on the `deliver_followup_required` resume path, the re-stamp runs as a SPECIAL PRE-FRESHNESS step (before the generic `_authority_fresh()` gate) — a generic freshness check ahead of the re-stamp would over-pause the legitimate insertion.
