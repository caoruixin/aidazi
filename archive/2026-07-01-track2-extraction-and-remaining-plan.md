...
## OPEN DECISION (needs human answer before finalizing TD6)
The TD6 re-stamp is currently designed as **in-memory per invocation** (`run_campaign_entry` loads the plan from file each run and never writes it back; the re-stamp is a deterministic function of (stored signed envelope, live plan) recomputed as the pre-freshness step on `deliver_followup_required`). Alternative = **persist** the re-stamped plan to the plan file (a CLI change outside `campaign.py`). Recommendation: keep in-memory (no plan-file write-back) unless persistence is explicitly required.
