...
3. **Wire the harness root entry** — add a root `CLAUDE.md` with `@AGENTS.md` (or your harness's equivalent import) so a fresh session loads your `AGENTS.md` Control Plane entry; role sessions then load the always-load governance kernel trio (full canonical on-demand).
