...
The DETERMINISTIC execution-fault sibling of §1.7-F. Where §1.7-F closes *completeness* gaps from coverage/ledger facts, §1.7-G repairs *browser_e2e execution faults* from framework-observed executor facts. Distinct from the interpretive quality `fix_required` channel (§1.7-C / §3.5, UNCHANGED), the autonomous browser_e2e remediation lane triggers ONLY from **DETERMINISTIC, framework-observed, criterion-bound execution facts** — one or more signed functional-checklist criteria whose `CriterionResult.executor_status ∈ {fail, error}` in a **FULL managed real-execution run** (the native `external_test_runner` executor with framework-owned provenance, `archive/2026-07-05-native-e2e-managed-autonomous-remediation-design.md` §3–§4). The failure brief is **generated by the framework from these executor facts**, NOT from the LLM Acceptance verdict. The interpretive Acceptance `fix_required` / `needs_human` / unbound `failure_briefs` are **NOT a §1.7-G trigger** and **route to the existing §3.5 human gate**; a reporter-`skipped` bound criterion **routes to §3.5-human** (the test did not assert — a human judgment, never a silent pass), and an `unmapped` signed criterion (no bound test) is a **pre-publication contract HALT** for a runner-contract change (never emitted as `skipped`, never a code-fault).

Under `autonomy.level: human_on_the_loop` (or higher), IFF the milestone carries an explicit **SIGNED `authority.e2e_remediation` budget** (resolved from charter `autonomy.e2e_remediation`, inside `charter_hash ⊂ H`), the orchestrator MAY, WITHOUT a fresh human-confirm checkpoint, dispatch a **bounded in-envelope Dev fix**, rerun the managed suite (authoritative re-judge = a FULL managed run, never a targeted `--grep` diagnostic probe), and re-enter Acceptance — IFF ALL of the following hold (deterministic, enforced at runtime):

1. **In-envelope containment (proven BEFORE the Dev fix, re-checked BEFORE rerun).** The framework-generated failure brief **binds each failing `criterion_id` to `{req_id, module, layer}`**; a deterministic **req_id-envelope check** proves the fix targets ⊆ (**F1 signed snapshot ∩ this milestone's signed `covers_req_ids`**) — DISTINCT from `post_gate1_scope_expansion` (modules/layers only); and an **observed-diff scope check** confirms the actual Dev diff touched only in-envelope modules/req_ids **BEFORE rerun**. If the **containment guarantee is mechanically unavailable** at runtime (the diff-based scope gate is unwired/unavailable), the lane does NOT dispatch an unbounded fix — it **FAILS CLOSED to the §3.5 human gate**. Autonomous remediation runs ONLY where the containment guarantee is mechanically present.

2. **Bounded, strictly-shrinking (persisted, enforced at runtime).** A signed `authority.e2e_remediation.max_rounds` cap per milestone (persisted `e2e_remediation_round` counter, enforced by `driver._check_budget` as a sibling of the review `fix_round` cap — the two counters are DISTINCT and do not double-count); each round's failing-criterion set MUST **strictly shrink** — a **PROPER SUBSET of the prior round** (persisted `failing_criteria_by_round`; proper-subset, NOT identical, which misses A/B churn); a round that INTRODUCES a new failing criterion is a **regression HALT**. Each successful round strictly shrinks the set, guaranteeing termination.

3. **Fail-closed escalation.** On any bound exceeded, any non-shrinking (non-proper-subset) round, or an unavailable containment guarantee, the orchestrator **HALTs and escalates** — it **never silently stops, never loops**.

Absent a signed budget, deterministic criterion failures **route to the §3.5 human gate exactly as today** (legacy-safe; no silent behavior change for existing adopters); **auto-dispatch is permitted only under `human_on_the_loop` or higher**. §1.7-G **iterates on executor FACTS** toward deterministic criterion-pass, **NEVER toward ship**: the final ship/reject stays the human #9 `advisory_acceptance_pass_signoff` gate (unchanged), **M3 stays advisory** (no auto-ship), and §1.7-G **grants NO authority to ship, to widen scope**, or to change authority.

**Why this is consistent with §1.7-D and §3.6, not a weakening**: like §1.7-F, §1.7-G names ONE more charter-pre-authorized decision — *repairing a deterministic, in-envelope execution fault* — bounded by clauses 1–3, and never ships. §3.6 forbids auto-iterating toward *ship* on an *uncalibrated* Acceptance verdict; §1.7-G auto-iterates on **deterministic executor facts** (per-criterion `executor_status`), never on the interpretive verdict, and always halts at #9 for the human ship decision — so §3.6's bar is preserved. A raise of any `authority.e2e_remediation` bound is an authority change: it flips `H` → `stale` → re-sign; a NORMAL remediation round adds no re-sign.
...
| **Dev Agent** | sprint-NNN-dev-prompt.md ready | self-contained dev prompt (per Δ-5 + Δ-9 prompt-artifact rules) | code edits + tests + `sprint-NNN-handoff.md` (§1-§11 dev fills; §12 reserved for deliver+human close verdict) | Human paste or charter-permitted orchestrator (workspace-write sandbox, network follows `charter.tooling.dev.network_access`, no git push) | Adopter choice via `charter.tooling.dev.agent_kind` — Codex (subscription-billed; cost-asymmetry caveat), Claude Code, or any tool-using coding agent | implementation |
...
**§1.7-G carve-out (scoped, does not weaken this gate)**: a browser_e2e milestone's DETERMINISTIC criterion-fail (`executor_status ∈ {fail, error}` from a full managed run) under a signed `authority.e2e_remediation` budget is dispatched autonomously via §1.7-G, NOT this human gate — the framework repairs a deterministic, in-envelope execution fault and re-judges, never shipping. The INTERPRETIVE `fix_required` / `needs_human` Acceptance verdict stays §3.5 human-gated at every autonomy level; only the deterministic, criterion-bound, in-envelope subset is carved out.
...
**§1.7-G reconciliation**: §1.7-G's autonomous browser_e2e remediation auto-iterates on **deterministic executor FACTS** (per-criterion `executor_status`) toward criterion-pass, never on the interpretive Acceptance verdict toward ship. §3.6's bar — no auto-iterating an *uncalibrated authoritative* verdict toward ship — is therefore preserved: §1.7-G never ships (it always halts at the #9 `advisory_acceptance_pass_signoff` gate for the human ship decision), and M3 stays advisory regardless.
