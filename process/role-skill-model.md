...
1. **Whitelist + sandbox inheritance (transitive)** — any role skill or sub-agent operates within the spawning role's tool whitelist and sandbox. Concretely:
   ...
   - Dev sub-agents inherit workspace-write / the spawning role's network grant / no git push / no holdout-eval read (`role-cards/dev-agent.md` §1).
...
| **Dev** | compact dev prompt contract (`templates/compact-dev-prompt.md`); handoff template | **primary mount point for industry stack-specialist skills/subagents** (frontend, backend, database, test-authoring) | MAY fan out within sandbox; all sub-agents inherit workspace-write / role network grant / no-push / no-holdout-read |
