...
- **Δ-19 `customer_disposition` is Customer authority for decided values, never LLM** (`archive/2026-06-23-requirement-ledger-design.md` §4.E; Constitution §1.3/§1.7). No agent/engine writes a **decided** requirement-ledger `customer_disposition` — agents *propose*, only the Customer sets a decided value; an engine/agent MAY seed the undecided sentinel `pending` on a NEW item, never a decided value and never a transition out of `pending`. `delivery_status` is a DERIVED `scope_report` projection, never authored/written back (preserves the Acceptance role boundary). Enforced by construction (there is no engine write path to a decided disposition).
