...
This is **necessary, not authoritative**: it is a structural Definition-of-Done item, NOT a substitute for the orchestrator's independent, hash-anchored browser evidence run (which is a separate gate you do not perform). It catches the cheap "the app doesn't even start on the happy path" failure at your seam. The orchestrator checks `docs/self-smoke.json` for PRESENCE (not correctness) at the `e2e_evidence_pending` gate. It is scoped to `browser_e2e` milestones; a static milestone does not require it.

**Still author it — the absence path is now autonomous, not a routine human halt (§6b).** For the managed `external_test_runner` class the check is SUBSUMED (the managed run's app-start + real journey with framework provenance is the attestation), so you do not need a separate `docs/self-smoke.json`. For the in-process `playwright` class with a signed `e2e_remediation` budget, an absent/malformed file triggers a bounded AUTONOMOUS Dev re-dispatch (you are re-dispatched to author it) rather than halting for a human; only an exhausted budget or an out-of-envelope change HALTs. Everything else keeps the resumable `gate_hard_fail`. Authoring it up front (belt-and-suspenders) avoids the extra round.
